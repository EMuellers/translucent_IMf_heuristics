...
class InductiveMinerFrameworkTranslucent(ABC, Generic[T]):
    """
    ...
    def _recurse(self, tree: ProcessTree, objs: List[T], level, parameters: Optional[Dict[str, Any]] = None):
        children = [self.apply(obj, parameters=parameters, level=level+1) for obj in objs]
        for c in children:
            c.parent = tree
