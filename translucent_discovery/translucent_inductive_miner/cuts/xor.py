...
class ExclusiveChoiceCut(Cut[T], ABC, Generic[T]):
    ...
    def holds(cls, obj: T, parameters: Optional[Dict[str, Any]] = None) -> Optional[List[Collection[Any]]]:
        ...
        '''
        #Elias: Abgesehen von tDFG if Abfrage identisch zu pm4py
        if parameters["tDFG"]:
...
#Elias: Entspricht Code von pm4py, nur return geändert für korrekte DataStructure creation
...
class ExclusiveChoiceCutTranslucentTCL(ExclusiveChoiceCut[IMDataStructureTranslucent]):
    ...
    def project(cls, obj: IMDataStructureTranslucent, groups: List[Collection[Any]], parameters: Optional[Dict[str, Any]] = None) -> List[IMDataStructureTranslucent]:
        ...
        return list(map(lambda l: IMDataStructureTranslucent(None, l, frequent=False, parameters=parameters, self_loop_info=obj._translucent_self_loops), logs)) # frequent=False!