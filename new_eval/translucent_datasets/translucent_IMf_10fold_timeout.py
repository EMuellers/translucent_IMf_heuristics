...
def evaluate_single_filter(f: float, train_log, test_log,
    ...
    # Generate an immutable hashable tuple representing the unique configuration dictionary
    ...
    fit_tf_key = (log_name, noise_type, config_key_part, f, "IMtf", "fitness")
    ...
    fit_ts_key = (log_name, noise_type, config_key_part, f, "IMts", "fitness")
    ...
    tfit_tf_key = (log_name, noise_type, config_key_part, f, "IMtf", "translucent_fitness")
    if timeout_registry is not None and timeout_registry.get(tfit_tf_key):
        print(f" [SKIPPED] calculate_log_fitness IMtf (prior timeout) | log={log_name}, noise={noise_type}, fold={fold_idx}, f={f}", flush=True)
        ...
        if not _run_fn_in_process(_calculate_log_fitness_wrapper, (test_log, net_tf, im_tf, fm_tf), _tfit_tf):
            print(f" [TIMEOUT] calculate_log_fitness IMtf | log={log_name}, noise={noise_type}, fold={fold_idx}, f={f}", flush=True)
            if timeout_registry is not None:
    ...
    tfit_ts_key = (log_name, noise_type, config_key_part, f, "IMts", "translucent_fitness")
    if timeout_registry is not None and timeout_registry.get(tfit_ts_key):
        print(f" [SKIPPED] calculate_log_fitness IMts (prior timeout) | log={log_name}, noise={noise_type}, fold={fold_idx}, f={f}", flush=True)
        ...
        if not _run_fn_in_process(_calculate_log_fitness_wrapper, (test_log, net_ts, im_ts, fm_ts), _tfit_ts):
            print(f" [TIMEOUT] calculate_log_fitness IMts | log={log_name}, noise={noise_type}, fold={fold_idx}, f={f}", flush=True)
            if timeout_registry is not None:
