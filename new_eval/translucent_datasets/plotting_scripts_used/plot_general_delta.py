...
def plot_metric_group(df_dict, log_name, noise_type, metric_group_name,
    ...
    for config_key, df in df_dict.items():
        # Aggregate across folds using custom_mean
        ...
        # Convert 0.0 to np.nan so Matplotlib doesn't plot these points
    ...
    # Replace 0.0 with np.nan for IMf
    ...
    # Calculate Petrify normal and trans values (output 0.0 if any fold fails)
    ...
    # Only plot Petrify if the baseline is > 0
    if petrify_normal_val > 0:
        ax1.plot(
    ...
    # Aggregate "No Heuristics" baseline rows across folds using custom_mean
    IM_trans_no_heur_df_avg = IM_trans_no_heur_df.groupby("threshold").agg(custom_mean).reset_index().sort_values("threshold")
    
    # Replace 0.0 with np.nan for No Heuristics baseline
    for col in normal_cols:
        IM_trans_no_heur_df_avg[col] = IM_trans_no_heur_df_avg[col].replace(0.0, np.nan)
    if translucent_cols:
        for col in translucent_cols:
            IM_trans_no_heur_df_avg[col] = IM_trans_no_heur_df_avg[col].replace(0.0, np.nan)
    ...
    ax1.plot(
        IM_trans_no_heur_df_avg["threshold"],
        IM_trans_no_heur_df_avg[normal_cols[0]],
        linestyle="-",
        marker="^",
    ...
    if translucent_cols:
        ax1.plot(
            IM_trans_no_heur_df_avg["threshold"],
            IM_trans_no_heur_df_avg[translucent_cols[0]],
            linestyle=":",
            marker="^",
            color=color_IMf_trans_no_heur,
            zorder=20
    ...
    ax2.plot(
        IM_trans_no_heur_df_avg["threshold"],
        IM_trans_no_heur_df_avg[normal_cols[1]],
        linestyle="-",
        marker="o",
        markersize=4,
        color=color_IMf_trans_no_heur,
    ...
    if translucent_cols:
        ax2.plot(
            IM_trans_no_heur_df_avg["threshold"],
            IM_trans_no_heur_df_avg[translucent_cols[1]],
            linestyle=":",
            marker="o",
            markersize=4,
            color=color_IMf_trans_no_heur,
