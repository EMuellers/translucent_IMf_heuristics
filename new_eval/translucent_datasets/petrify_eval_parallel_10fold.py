...
def evaluate_single_fold(train_log, test_log, log_name: str, noise_label: str, fold_idx: int) -> dict:
    ...
    # --- All conformance metrics evaluated on the held-out test split ----
    _fit = Value("d", -1.0)
    ...
    if not _run_fn_in_process(_precision_alignments_wrapper, (test_log, net, im, fm), _prec):
        print(f" [TIMEOUT] precision_alignments | log={log_name}, noise={noise_label}, fold={fold_idx}", flush=True)
    precision = _prec.value

    translucent_precision = _run_returning_value_in_process(
        _translucent_precision_score_wrapper, (test_log, net, im, fm))
    if translucent_precision is None:
        print(f" [TIMEOUT] translucent_precision_score | log={log_name}, noise={noise_label}, fold={fold_idx}", flush=True)
...
def _central_task_worker(task_queue: Queue, out_queue: Queue) -> None:
    while True:
        ...
        try:
            result_data = evaluate_single_fold(train_log, test_log, log_name, noise_label, fold_idx)
