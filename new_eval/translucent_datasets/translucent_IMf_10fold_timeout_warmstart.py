...
def _get_alignment_fitness_wrapper(log, net, im, fm) -> float:
    return get_alignment_fitness_with_processtree(log, net, im, fm)
...
def _pnml_path(log_name: str, noise_label: str, config: dict,
    ...
    folder = Path(
        f"new_eval/translucent_datasets/{log_name}/pnml/translucent_minor_10fold/{noise_label}/"
...
def evaluate_single_filter(f: float, train_log, test_log,
                            ...
                            fold_idx: int, parameters: dict = None, timeout_registry = None) -> tuple:
    parameters = parameters or {}
    
    # Float precision safeguard for timeout dict keys
    f_key = round(float(f), 4)
    config_key_part = tuple(sorted(parameters.items()))
    ...
    # --- 1. Alignment Fitness IMtf ---
    fit_tf_key = (log_name, noise_type, config_key_part, f_key, "IMtf", "fitness")
    if timeout_registry is not None and timeout_registry.get(fit_tf_key):
        print(f" [SKIPPED] fitness IMtf (prior timeout) | log={log_name}, noise={noise_type}, fold={fold_idx}, f={f}", flush=True)
        fitness_tf = -1.0
    else:
        _fit_tf = Value("d", -1.0)
        if not _run_fn_in_process(_get_alignment_fitness_wrapper, (test_log, net_tf, im_tf, fm_tf), _fit_tf):
            print(f" [TIMEOUT] fitness IMtf | log={log_name}, noise={noise_type}, fold={fold_idx}, f={f}", flush=True)
            if timeout_registry is not None:
                timeout_registry[fit_tf_key] = True
            fitness_tf = -1.0
        else:
            fitness_tf = _fit_tf.value

    # --- 2. Alignment Fitness IMts ---
    fit_ts_key = (log_name, noise_type, config_key_part, f_key, "IMts", "fitness")
    if timeout_registry is not None and timeout_registry.get(fit_ts_key):
        print(f" [SKIPPED] fitness IMts (prior timeout) | log={log_name}, noise={noise_type}, fold={fold_idx}, f={f}", flush=True)
        fitness_ts = -1.0
    else:
        _fit_ts = Value("d", -1.0)
        if not _run_fn_in_process(_get_alignment_fitness_wrapper, (test_log, net_ts, im_ts, fm_ts), _fit_ts):
            print(f" [TIMEOUT] fitness IMts | log={log_name}, noise={noise_type}, fold={fold_idx}, f={f}", flush=True)
            if timeout_registry is not None:
                timeout_registry[fit_ts_key] = True
            fitness_ts = -1.0
        else:
            fitness_ts = _fit_ts.value
    
    # --- 3. Translucent Fitness IMtf ---
    tfit_tf_key = (log_name, noise_type, config_key_part, f_key, "IMtf", "translucent_fitness")
    if timeout_registry is not None and timeout_registry.get(tfit_tf_key):
        print(f" [SKIPPED] calc_log_fitness IMtf (prior timeout) | log={log_name}, noise={noise_type}, fold={fold_idx}, f={f}", flush=True)
        translucent_fitness_tf = -1.0
    else:
        _tfit_tf = Value("d", -1.0)
        if not _run_fn_in_process(_calculate_log_fitness_wrapper, (test_log, net_tf, im_tf, fm_tf), _tfit_tf):
            print(f" [TIMEOUT] calc_log_fitness IMtf | log={log_name}, noise={noise_type}, fold={fold_idx}, f={f}", flush=True)
            if timeout_registry is not None:
                timeout_registry[tfit_tf_key] = True
            translucent_fitness_tf = -1.0
        else:
            translucent_fitness_tf = _tfit_tf.value

    # --- 4. Translucent Fitness IMts ---
    tfit_ts_key = (log_name, noise_type, config_key_part, f_key, "IMts", "translucent_fitness")
    if timeout_registry is not None and timeout_registry.get(tfit_ts_key):
        print(f" [SKIPPED] calc_log_fitness IMts (prior timeout) | log={log_name}, noise={noise_type}, fold={fold_idx}, f={f}", flush=True)
        translucent_fitness_ts = -1.0
    else:
        _tfit_ts = Value("d", -1.0)
        if not _run_fn_in_process(_calculate_log_fitness_wrapper, (test_log, net_ts, im_ts, fm_ts), _tfit_ts):
            print(f" [TIMEOUT] calc_log_fitness IMts | log={log_name}, noise={noise_type}, fold={fold_idx}, f={f}", flush=True)
            if timeout_registry is not None:
                timeout_registry[tfit_ts_key] = True
            translucent_fitness_ts = -1.0
        else:
    ...
    return f, {
        ...
        "fitness_tf": fitness_tf, "precision_tf": 0, "f_1_score_tf": 0,
        "fitness_ts": fitness_ts, "precision_ts": 0, "f_1_score_ts": 0,
        ...
        "translucent_f_1_score_tf": 0, "translucent_f_1_score_ts": 0,
        "simplicity_tf": simplicity_tf, "simplicity_ts": simplicity_ts, "failed": False,
    }

# ---------------------------------------------------------------------------
# Worker & Dispatch setup
# ---------------------------------------------------------------------------
...
# --- NEW: Warmstart Parser ---
def _get_warmstart_state(log_names, noise_types, parameter_configs):
    """
    Parses existing CSV outputs to determine which iterations have completed,
    and identifies which configurations previously triggered a timeout.
    """
    completed_tasks = set()
    prior_timeouts = {}
    ...
    for log_name in log_names:
        for noise_type in noise_types:
            ...
            for config in parameter_configs:
                config_str = prettify_config_for_file_name(config)
                config_tuple = tuple(sorted(config.items()))
                results_path = Path(
                    f"new_eval/translucent_datasets/{log_name}/results/"
                    f"translucent_minor_10fold/{noise_label}/IMf_10fold_{config_str}.csv"
                )
                ...
                if results_path.exists():
                    try:
                        ...
                        for _, row in df.iterrows():
                            f_val = row.get("threshold")
                            fold = row.get("fold")
                            
                            # Handle corrupted rows gracefully
                            if pd.isna(f_val) or pd.isna(fold):
                                continue
                            ...
                            completed_tasks.add((log_name, noise_label, config_str, f_key, fold))

                            # Re-establish previous timeouts directly from the resulting metrics
                            if row.get("fitness_tf") == -1.0:
                                prior_timeouts[(log_name, noise_label, config_tuple, f_key, "IMtf", "fitness")] = True
                            if row.get("fitness_ts") == -1.0:
                                prior_timeouts[(log_name, noise_label, config_tuple, f_key, "IMts", "fitness")] = True
                            if row.get("translucent_fitness_tf") == -1.0:
                                prior_timeouts[(log_name, noise_label, config_tuple, f_key, "IMtf", "translucent_fitness")] = True
                            if row.get("translucent_fitness_ts") == -1.0:
                                prior_timeouts[(log_name, noise_label, config_tuple, f_key, "IMts", "translucent_fitness")] = True

                    except Exception as exc:
                        print(f" [WARN] Could not parse existing results for {results_path.name}: {exc}")

    return completed_tasks, prior_timeouts

def _build_task_list(log_names, noise_types, fold_map, parameter_configs, filter_values, completed_tasks):
    tasks = []
    for log_name in log_names:
        for noise_type in noise_types:
            noise_label = noise_type if noise_type else "base"
            for fold_idx, train_log, test_log in fold_map[(log_name, noise_label)]:
                for config in parameter_configs:
                    ...
                    for f in filter_values:
                        f_key = round(float(f), 4)
                        # Skip if already logged in CSV
                        if (log_name, noise_label, config_str, f_key, fold_idx) in completed_tasks:
                            continue
    ...
    while True:
        ...
        try:
            f_out, result_data = evaluate_single_filter(
                f, train_log, test_log, log_name, noise_label, fold_idx, config, timeout_registry)
...
def run_evaluation(log_names, noise_types, path_to_log_fn, filter_values,
    ...
    # --- NEW: Engage Warmstart Logic ---
    completed_tasks, prior_timeouts = _get_warmstart_state(log_names, noise_types, parameter_configs)
    print(f"\n[WARMSTART] Discovered {len(completed_tasks)} finished fold calculations.")
    print(f"[WARMSTART] Discovered {len(prior_timeouts)} prior timeouts to fast-skip.\n")
    ...
    for log_name in log_names:
        for noise_type in noise_types:
            ...
            for config in parameter_configs:
                ...
                results_folder = Path(
                    f"new_eval/translucent_datasets/{log_name}/results/"
                    f"translucent_minor_10fold/{noise_label}"
                )
    ...
    # Pass completed tasks into the builder
    tasks = _build_task_list(log_names, noise_types, fold_map, parameter_configs, filter_values, completed_tasks)
    total_tasks = len(tasks)
    print(f"[INIT] {total_tasks} jobs queued across {N_WORKERS} workers\n")

    if total_tasks == 0:
        print("\n[DONE] All tasks completed via warmstart! Exiting cleanly.")
        for q in result_queues.values(): q.put("Ende")
        for writer in writer_processes.values(): writer.join()
        return
    ...
    # Initialize the timeout dict loaded with prior timeouts
    manager = mp.Manager()
    timeout_registry = manager.dict(prior_timeouts)
