...
def _get_alignment_fitness_wrapper(log, net, im, fm):
...
def evaluate_single_filter(f: float, train_log, test_log,
                            log_name: str, noise_type: str, fold_idx: int, timeout_registry) -> tuple:
    ...
    fit_key = (log_name, noise_type, f, "fitness")
    if timeout_registry.get(fit_key):
        print(f" [SKIPPED] fitness (prior timeout) | log={log_name}, noise={noise_type}, fold={fold_idx}, f={f}", flush=True)
        fitness = -1.0
    else:
        res_fit = _run_returning_pair_in_process(_get_alignment_fitness_wrapper, (test_log, net, im, fm))
        if res_fit is None:
            print(f" [TIMEOUT] fitness | log={log_name}, noise={noise_type}, fold={fold_idx}, f={f}", flush=True)
    ...
    tfit_key = (log_name, noise_type, f, "translucent_fitness")
    if timeout_registry.get(tfit_key):
        print(f" [SKIPPED] calculate_log_fitness (prior timeout) | log={log_name}, noise={noise_type}, fold={fold_idx}, f={f}", flush=True)
...
def _central_task_worker(task_queue: Queue, out_queue: Queue, timeout_registry) -> None:
    while True:
        ...
        try:
            f_out, result_data = evaluate_single_filter(f, train_log, test_log, log_name, noise_label, fold_idx, timeout_registry)
